    ...
    <group>
      {tiers.map((tier, i) => (
        <mesh key={tier.id} ref={refs[i]} position={[0, tier.y, 0]}>
          <cylinderGeometry
            args={[tier.radius, tier.radius, tier.height, 64]}
          />
...
function LaptopModel() {
  ...
  useFrame((state) => {
    if (screenRef.current) {
      // Subtle lid tilt: sin wave 0.03 amplitude
    ...
    if (screenGlowRef.current) {
      ...
      mat.emissiveIntensity =
        1.6 + Math.sin(state.clock.elapsedTime * 1.2) * 0.2;
    }
  });
  ...
  return (
    <group>
      ...
      {/* Screen panel with tilt animation */}
      <group ref={screenRef} position={[0, 0.6, 0]}>
        ...
        {/* Screen decorative bars (UI chrome) */}
        {(
          [
...
function PhoneModel() {
  ...
  // Register listener once via a custom setup pattern
  ...
  useFrame((state) => {
    if (phoneGroupRef.current) {
      // Tilt ±8 degrees based on mouse X
    ...
    if (screenRef.current) {
      ...
      mat.emissiveIntensity =
        1.3 + Math.sin(state.clock.elapsedTime * 0.9) * 0.2;
    }
  });

  // Analytics bar heights (mini chart)
...
function Particles() {
  ...
  const { positions, colors, phases } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Gold: #D4A017 → r:0.831 g:0.627 b:0.09
      // Purple: #4A2C9E → r:0.29 g:0.17 b:0.62
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      pos.setY(
        i,
        origPositions[i * 3 + 1] + Math.sin(t * 0.22 + phase * 1.3) * 0.28,
      );
      pos.setZ(
        i,
        origPositions[i * 3 + 2] + Math.cos(t * 0.15 + phase * 0.7) * 0.2,
      );
    }
...
function CameraRig() {
  ...
  useFrame(() => {
    // Lerp mouse parallax
    targetRef.current.x +=
      (mouseRef.current.x * 0.25 - targetRef.current.x) * 0.08;
    targetRef.current.y +=
      (mouseRef.current.y * 0.2 - targetRef.current.y) * 0.08;

    camera.position.x = targetRef.current.x;
    camera.position.y = targetRef.current.y;

    // Scroll-based zoom + tilt
  ...
  return (
    ...
    >
      <div className="text-center space-y-4 p-8">
        <div className="w-24 h-16 mx-auto bg-brand-purple/60 rounded-xl flex items-center justify-center text-4xl">
          💻
        </div>
        <div className="flex gap-4 justify-center">
          {["📱", "⭐", "📊", "🚀"].map((icon) => (
            <div
              key={icon}
              className="w-10 h-10 bg-brand-gold/20 rounded-lg flex items-center justify-center text-xl animate-float"
            >
              {icon}
            </div>
          ))}
...
function SceneLoader() {
  return (
    <div className="w-full h-full rounded-2xl flex items-center justify-center bg-brand-card-dark/50">
