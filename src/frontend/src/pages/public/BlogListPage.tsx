...
function CategoryBadge({
  category,
  hover,
}: { category: string; hover?: boolean }) {
  const colorClass =
    CATEGORY_COLORS[category] ||
    "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/20 dark:text-amber-300 dark:border-amber-800";
  return (
    <span
      className={`inline-block text-xs font-mono font-semibold px-2.5 py-1 rounded-full border transition-all duration-200 ${colorClass} ${
        hover ? "brightness-110 saturate-150" : ""
      }`}
    >
      {category}
    </span>
  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <p className="text-sm text-brand-text-secondary dark:text-white/60 line-clamp-2 flex-1 leading-relaxed">
          {post.excerpt}
        </p>
    ...
    <ScrollReveal delay={index * 0.12}>
      ...
      >
        ...
        >
          ...
          <div className="absolute top-4 right-4 bg-white/20 backdrop-blur-sm rounded-full px-3 py-1 text-xs text-white font-mono">
            {post.readTime} read
          </div>
        </div>
        <div className="p-5">
          <h3 className="font-heading font-semibold text-lg text-brand-text-primary dark:text-white mb-2 line-clamp-2 group-hover:text-brand-purple dark:group-hover:text-brand-gold transition-colors leading-snug">
            {post.title}
          </h3>
          <p className="text-sm text-brand-text-secondary dark:text-white/60 line-clamp-2 leading-relaxed mb-4">
            {post.excerpt}
          </p>
...
export default function BlogListPage() {
  ...
  /* Sentinel to detect sticky filter bar */
  useEffect(() => {
  ...
  const allPosts: DisplayPost[] =
    backendPosts.length > 0 ? backendPosts : MOCK_POSTS;
    ...
    <div className="min-h-screen bg-brand-bg-light dark:bg-brand-bg-dark">
      ...
      <section className="gradient-mesh py-24 relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute bottom-1/4 right-1/4 w-60 h-60 bg-brand-gold/8 rounded-full blur-3xl" />
        </div>
        <div className="max-w-4xl mx-auto px-4 text-center relative">
          <ScrollReveal>
            ...
            <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-brand-text-primary dark:text-white mt-3 mb-5 leading-tight">
              ...
              <span className="text-brand-purple dark:text-brand-gold">
                Industry Knowledge
              </span>
            </h1>
            <p className="text-lg text-brand-text-secondary dark:text-white/70 max-w-2xl mx-auto leading-relaxed">
              Expert insights on social media, healthcare marketing, SEO, and
      ...
      {/* Sentinel div — used to detect when filter bar becomes sticky */}
      <div ref={sentinelRef} className="h-px" />

      {/* ── Sticky Category Filter ─────────────────────────────────────────── */}
      <div
        className={`sticky top-16 z-40 bg-white/95 dark:bg-brand-bg-dark/95 backdrop-blur-md border-b border-brand-border-light dark:border-white/10 transition-shadow duration-300 ${
          filterStuck ? "shadow-md" : ""
        }`}
        ...
        {selectedCategory === "All" && (
          <section data-ocid="blog.featured.section">
            <ScrollReveal>
              ...
              <h2 className="font-heading text-2xl font-bold text-brand-text-primary dark:text-white mt-2 mb-8">
                Must-Read Articles
              </h2>
            </ScrollReveal>
            {loading ? (
        ...
        <section data-ocid="blog.grid.section">
          ...
          {loading ? (
            ...
            >
              ...
              <h3 className="font-heading text-xl font-semibold text-brand-text-primary dark:text-white mb-2">
            ...
            >
              <AnimatePresence mode="wait">
                {pagedPosts.map((post, i) => {
                  ...
                  const showCta =
                    (i + 1) % 6 === 0 && i < pagedPosts.length - 1;
                  return (
                    <Fragment key={post.id}>
                      <BlogCard post={post} index={globalIdx} />
                      {showCta && <InlineCTACard onOpen={openModal} />}
                    </Fragment>
                  );
                })}
              </AnimatePresence>
        ...
        <ScrollReveal>
          <section className="bg-gradient-to-br from-brand-purple to-brand-purple-2 rounded-3xl p-10 text-center relative overflow-hidden">
            ...
            <div className="relative">
              <span className="text-brand-gold-light text-xs font-mono uppercase tracking-widest">
                GROW YOUR BUSINESS
              </span>
              ...
              <p className="text-white/70 max-w-xl mx-auto mb-8 text-lg">
                Let our team handle the digital strategy while you focus on your
                business.
              </p>
              <motion.button
                type="button"
