...
function HeroSection({ service, onBookClick }: { service: ServicePage; onBookClick: () => void }) {
  return (
    <section className="relative overflow-hidden min-h-[540px] flex items-center py-20 px-4">
      ...
      <motion.div
        className="absolute inset-0 pointer-events-none"
        style={{ background: `radial-gradient(ellipse at 50% 50%, ${service.hero.gradientFrom}22 0%, transparent 70%)` }}
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div>
          <nav className="flex items-center gap-2 text-sm text-brand-purple/60 dark:text-white/50 mb-4 font-body">
            ...
            <span className="text-brand-purple dark:text-white/80">
              {service.hero.categoryLabel.replace(/ MANAGEMENT| & DESIGN| & DEVELOPMENT/g, '').replace(/[A-Z]+/, (s) => s.charAt(0) + s.slice(1).toLowerCase())}
            </span>
          </nav>

          <ScrollReveal>
            <span className={sectionLabelClass}>{service.hero.categoryLabel}</span>
          </ScrollReveal>
          ...
          <ScrollReveal delay={0.3}>
            <div className="mt-8 flex flex-wrap gap-4">
              ...
              >
                Book Consultation
                <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <ScrollReveal direction="scale" delay={0.2} className="hidden lg:flex justify-center">
          <div
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <ScrollReveal direction="left">
            <div className="h-full border border-red-200 dark:border-red-900/40 bg-red-50/80 dark:bg-red-950/20 rounded-2xl p-8 flex flex-col gap-4">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-heading text-xl font-semibold text-red-700 dark:text-red-400">The Challenge</h3>
              </div>
              <p className="font-body text-red-800/80 dark:text-red-300/80 leading-relaxed text-base">{service.challenge.problem}</p>
            </div>
          </ScrollReveal>
          <ScrollReveal direction="right">
            <div className="h-full border border-green-200 dark:border-green-900/40 bg-green-50/80 dark:bg-green-950/20 rounded-2xl p-8 flex flex-col gap-4">
              <div className="flex items-center gap-3">
                <span className="text-3xl">✅</span>
                <h3 className="font-heading text-xl font-semibold text-green-700 dark:text-green-400">Our Solution</h3>
              </div>
              <p className="font-body text-green-800/80 dark:text-green-300/80 leading-relaxed text-base">{service.challenge.solution}</p>
            </div>
          </ScrollReveal>
        </div>
      </div>
    ...
    <section className="py-20 px-4 bg-brand-bg-light dark:bg-brand-bg-dark">
      <div className="max-w-7xl mx-auto">
        <ScrollReveal className="text-center mb-14">
          ...
          <h2 className="mt-2 font-display text-3xl lg:text-4xl font-bold text-brand-purple dark:text-white">{service.whatWeOffer.heading}</h2>
        </ScrollReveal>
        <motion.div
        ...
        >
          {service.whatWeOffer.items.map((item, idx) => (
            ...
            >
              <div className="w-12 h-12 rounded-xl bg-brand-purple/10 dark:bg-white/10 flex items-center justify-center text-2xl group-hover:scale-110 transition-transform duration-300">{item.icon}</div>
              <div>
                <h3 className="font-heading font-semibold text-brand-purple dark:text-white text-lg leading-snug">{item.title}</h3>
                <p className="mt-1 font-body text-sm text-brand-purple/60 dark:text-white/60 leading-relaxed">{item.description}</p>
              </div>
              <ul className="mt-auto space-y-1.5">
                {item.includes.map((inc) => (
                  <li key={inc} className="flex items-start gap-2 text-sm font-body text-brand-purple/70 dark:text-white/70">
                    <span className="mt-0.5 w-4 h-4 shrink-0 rounded-full bg-brand-gold/20 flex items-center justify-center">
                      <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" className="w-2.5 h-2.5 text-brand-gold" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <section className="py-20 px-4 bg-white dark:bg-brand-card-dark overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <ScrollReveal className="text-center mb-14">
          ...
          <h2 className="mt-2 font-display text-3xl lg:text-4xl font-bold text-brand-purple dark:text-white">How We Deliver Results</h2>
        </ScrollReveal>
        <div className="hidden lg:flex items-start gap-0">
          {steps.map((step, idx) => (
            <ScrollReveal key={step.number} delay={idx * 0.1} className="flex-1">
              <div className="relative flex flex-col items-center text-center px-4">
                {idx < steps.length - 1 && <div className="absolute top-7 left-[calc(50%+1.5rem)] right-0 h-px border-t-2 border-dashed border-brand-gold/40" />}
                <div className="w-14 h-14 rounded-full bg-brand-gold flex items-center justify-center font-mono font-bold text-white text-lg shadow-[0_0_20px_#D4A01744] shrink-0 z-10">{step.number}</div>
                <h3 className="mt-4 font-heading font-semibold text-brand-purple dark:text-white text-sm leading-snug">{step.title}</h3>
                <p className="mt-2 text-xs font-body text-brand-purple/60 dark:text-white/60 leading-relaxed">{step.description}</p>
              </div>
            </ScrollReveal>
          ))}
        </div>
          ...
          {steps.map((step, idx) => (
            <ScrollReveal key={step.number} delay={idx * 0.1}>
              <div className="relative flex gap-5 pb-8">
                {idx < steps.length - 1 && <div className="absolute left-7 top-14 bottom-0 w-px border-l-2 border-dashed border-brand-gold/40" />}
                <div className="w-14 h-14 rounded-full bg-brand-gold flex items-center justify-center font-mono font-bold text-white text-lg shadow-[0_0_20px_#D4A01744] shrink-0">{step.number}</div>
                <div className="pt-2">
                  <h3 className="font-heading font-semibold text-brand-purple dark:text-white text-base">{step.title}</h3>
                  <p className="mt-1 text-sm font-body text-brand-purple/60 dark:text-white/60 leading-relaxed">{step.description}</p>
                </div>
              </div>
            </ScrollReveal>
          ))}
...
function BenefitsSection({ service }: { service: ServicePage }) {
  return (
    <section className="py-20 px-4 bg-brand-purple">
      <div className="max-w-7xl mx-auto">
        <ScrollReveal className="text-center mb-14">
          <span className="text-brand-gold text-xs font-mono uppercase tracking-widest">YOUR ADVANTAGE</span>
          <h2 className="mt-2 font-display text-3xl lg:text-4xl font-bold text-white">{service.benefits.heading}</h2>
        </ScrollReveal>
        <motion.div
    ...
    <section className="py-16 px-4 bg-brand-bg-light dark:bg-brand-bg-dark">
      <div className="max-w-7xl mx-auto">
        <ScrollReveal className="text-center mb-10">
          ...
          <h2 className="mt-2 font-display text-3xl font-bold text-brand-purple dark:text-white">Industries We Serve</h2>
        </ScrollReveal>
        <motion.div
...
function CTASection({ service, onBookClick }: { service: ServicePage; onBookClick: () => void }) {
  return (
    <section className="relative py-24 px-4 overflow-hidden bg-brand-purple-2 dark:bg-brand-card-dark">
      ...
      <div className="relative z-10 max-w-3xl mx-auto text-center">
        <ScrollReveal>
          <h2 className="font-display text-3xl lg:text-4xl font-bold text-white mb-4">{service.cta.heading}</h2>
          <p className="font-body text-white/70 text-lg leading-relaxed mb-10">{service.cta.subtext}</p>
      ...
      <PublicLayout>
        <div className="min-h-screen flex items-center justify-center bg-brand-bg-light dark:bg-brand-bg-dark">
          <div className="text-center">
            ...
            <h1 className="font-display text-3xl font-bold text-brand-text-primary dark:text-white mb-4">Service Not Found</h1>
            <Link href="/services" className="bg-brand-purple text-white px-6 py-3 rounded-full font-heading font-semibold hover:bg-brand-purple-2 transition-smooth">
              ← Back to Services
            </Link>
          </div>
        </div>
  ...
  return (
    <PublicLayout>
      <div data-ocid="service-detail.page" className="min-w-0">
        <HeroSection service={service} onBookClick={() => setModalOpen(true)} />
        ...
        <CTASection service={service} onBookClick={() => setModalOpen(true)} />
      </div>
